...
*/
enum MemoryType { MEMORY_CPU, MEMORY_GPU, MEMORY_CPU_PINNED };
...
class MemManager {
   ...
   private:
    ...
    std::unordered_map<std::string, std::tuple<void*, size_t, MemoryType>>*
        pointer_mapping_;
   ...
   public:
    ...
    MemManager(const std::string manager_name = "") {
        pointer_mapping_ =
            new std::unordered_map<std::string,
                                   std::tuple<void*, size_t, MemoryType>>();
    }
    // Destructor
    ~MemManager() {
    ...
    // Function to generate a string identifier for a pointer if non is
    // provided.
    std::string generateName(const void* ptr);
    // Function to allocate memory in pageable, pinned host, and device memories
    // and a name to address that memory chunk.
    void* allocate(const size_t size, MemoryType mem_type,
                   bool is_set_to_zero = false, std::string name = "");
